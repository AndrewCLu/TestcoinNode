...
func testSolveSpeed(targetFirstFour [4]byte, numIters int) {
	...
	fmt.Printf("Solved difficulty %v with average time %v seconds. Maximum solve time was %v seconds, minimum time was %v seconds.", util.HashToHexString(target), totalTime/int64(numIters), maxTime, minTime)
}
...
func main() {
	...
	testSolveSpeed([4]byte{0, 0, 15, 255}, 100)

	testSolveSpeed([4]byte{0, 0, 7, 255}, 25)

	testSolveSpeed([4]byte{0, 0, 3, 255}, 25)

	testSolveSpeed([4]byte{0, 0, 0, 255}, 25)

	testSolveSpeed([4]byte{0, 0, 0, 191}, 10)

	testSolveSpeed([4]byte{0, 0, 0, 127}, 10)

	testSolveSpeed([4]byte{0, 0, 0, 63}, 10)
}
